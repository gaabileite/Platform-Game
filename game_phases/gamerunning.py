...
def game_running(player, enemies, death, platforms, shots, collectables, camera, surface, game_manager, flag):
    ...
    for c in collectables:
        if c.image:
            surface.blit(c.image, camera.apply(c))

    # --- coleta de coletáveis ---
    collected = []
    for c in collectables:
        if player.check_collision(c):
            player.add_collectable(c)
            collected.append(c)
    for c in collected:
