...
while True:
    ...
    if game_manager.current_phase == 0:
        player, enemies, death, platforms, collectables, shots, game_manager, flag = gamestart(surface, player, enemies, death, platforms, collectables, shots, game_manager, flag)

    #GAME RUNNING: PHASE 1, 2, 3 (WIP/BACK)
    elif game_manager.current_phase in [1,2,3]:
        game_manager, last_phase = game_running(player, enemies, death, platforms, shots, collectables, camera, surface, game_manager, flag)

    #PHASE TRANSITION (WIP/FRONT)
    elif game_manager.current_phase == 4:
        player, enemies, death, platforms, collectables, shots, game_manager, flag = phase_transition(surface, player, enemies, death, platforms, collectables, shots, game_manager, flag, last_phase)

    #GAME OVER (OK)
    elif game_manager.current_phase == 5:
        player, enemies, death, platforms, collectables, shots, game_manager, flag = gameover(surface, player, enemies, death, platforms, collectables, shots, game_manager, flag)

    #GAME WON (OK)
    elif game_manager.current_phase == 6:
        player, enemies, death, platforms, collectables, shots, game_manager, flag = gamewon(surface, player, enemies, death, platforms, collectables, shots, game_manager, flag)
